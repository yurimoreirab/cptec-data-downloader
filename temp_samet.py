...
def download_netcdf_files(start_date=None, end_date=None, local_dir='./downloads'):
    if not os.path.isdir(local_dir):
        os.makedirs(local_dir, exist_ok=True)
    base_url = 'https://ftp.cptec.inpe.br/modelos/tempo/SAMeT/HOURLY/'
            ...
            if response.status_code == 200:
                ...
                if netcdf_links:
                    ...
                    for href in netcdf_links:
                        ...
                        # Extrair a hora: os dois últimos dígitos antes de .nc
                        hour_match = re.search(r'(\d{2})\.nc$', href)
                        hour = hour_match.group(1) if hour_match else '00'

                        local_file_path = os.path.join(local_path, f'SAMeT_{date_str}-T{hour}.nc')
...
def concatenate_netcdf_files_by_month(local_dir='./downloads', output_dir=None):
    """Concatena os arquivos NetCDF baixados em um único arquivo por mês."""
    started_at = time.time()
    ...
    for index, (year, month, monthly_files) in enumerate(month_jobs, start=1):
        ...
        output_file = os.path.join(output_year_dir, f'SAMeT_{year}-{month}.nc')
...
def main(start_date=None, end_date=None, local_dir='./SAMeT'):
    ...
    download_stats = download_netcdf_files(
    ...
    concat_stats = concatenate_netcdf_files_by_month(local_dir=os.path.join(local_dir, 'daily'),
                                                     output_dir=os.path.join(local_dir, 'monthly'))
